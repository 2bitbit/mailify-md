    ...
    async def _embed_katex_as_base64(self, page: Page) -> str:
        ...
        for i, element_handle in enumerate(katex_elements):
            ...
            try:
                ...
                img_tag = soup.new_tag("img", src=base64_data, style="vertical-align: middle; max-width: 100%;")
                bs_element.replace_with(img_tag)
                print(f"  - 公式 {i+1} 已成功转换为Base64图片。")
    ...
    <head>
        ...
        <style>
            ...
            .container {
                ...
                margin: 20px auto;
                padding: 20px 40px;
                border: 5px solid #00a549; /* 直接使用颜色值 */
                border-radius: 20px;
                background-color: #ffffff;
            }
            :not(pre)>code {
                color: #0082f1; /* 直接使用颜色值 */
            ...
            h1 {
                border-bottom: 3px solid #FF6D2D; /* 直接使用颜色值 */
                padding-bottom: 10px;
            }
        </style>
    </head>
